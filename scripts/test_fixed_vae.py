#!/usr/bin/env python3
"""
Test with community-finetuned VAE that fixes decoder issues.
From: https://huggingface.co/spacepxl/ltx-video-0.9-vae-finetune
"""
...
def main():
    ...
    parser.add_argument("--output", type=str, default="test_fixed_vae.mp4")
    ...
    print("="*60)
    print("Testing with Community-Fixed VAE")
    print("="*60)
    ...
    print("  - VAE (COMMUNITY-FIXED)...")
    try:
        vae = AutoencoderKLLTXVideo.from_pretrained(
            "spacepxl/ltx-video-0.9-vae-finetune",
            torch_dtype=torch.bfloat16
        )
        print("    ✓ Using community-finetuned VAE")
    except Exception as e:
        print(f"    ✗ Failed to load fixed VAE: {e}")
        print("    Falling back to original VAE...")
        vae = AutoencoderKLLTXVideo.from_pretrained(
            "Lightricks/LTX-Video",
            subfolder="vae",
            torch_dtype=torch.bfloat16
